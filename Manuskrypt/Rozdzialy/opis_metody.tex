...
W ramach tego eksperymentu zostanie przedstawiona szczegółowa analiza wyników oceny procesu gojenia z wykorzystaniem proponowanej w tej pracy metody. W pierwszej kolejności zostały przedstawione \textit{krzywe gojenia} tj. ocena w czasie poszczególnych parametrów ankiety. Porównane zostały wyniki metody automatycznej z wzorcem odniesienia ocenianym przez eksperta radiologa. Następnie przeanalizowano macierze błędu dla każdego z parametrów, jak również dla klas stanowiących zakres oceny: 0--7. Powyższe zestawienia znajdują się w kolejnych podsekcjach.
...
Parametr ten zgodnie z przedstawioną interpretacją związany jest z oceną pogrubienia ścięgna. Na początku procesu pogrubienie jest małe lub średnie (9--12 mm.), następnie w fazie poliferacji rośnie, co związane jest z przyrostem włókien ścięgnistych, by finalnie, w większości przypadków, ponownie zmaleć w wyniku przebudowy struktury i pozostawienia jedynie dobrze ukierunkowanych, wytrzymałych struktur. Dla pacjentów 115805 i 127752, zgodnie z oceną radiologa, proces przebiega z wyżej przedstawionym opisem. W przypadku pacjenta 140283 widoczna jest anomalia w 3 tyg. co może mieć związek z fluktuacjami np. nadmiernym obciążeniem ścięgna. Ponownie, jedynie w przypadku pacjenta 127108, gojenie przebiega odmiennie i po operacji pogrubienie rośnie, aż do 52 tyg. Średni błąd (MAE) osiągnięty dla oceny tego parametru jest najniższy spośród wszystkich sześciu, co widoczne jest w przebiegach krzywych gojenia, które przybliżają ocenę radiologa z MAE równym $0.56$. Anomalia z 3 tyg. występująca u pacjenta 140283 nie została dobrze wykryta i w rezultacie błąd maksymalny (MAX-AE) wyniósł $1.82$. W celu detekcji przyczyny konieczna jest dalsza analiza i w pierwszej kolejności weryfikacja, czy błąd popełniony został przez ocenę radiologa, czy metodę.
...
Parametr ten związany jest z oceną granic pomiędzy ROI, a zewnętrznymi tkankami. Ścięgno po zerwaniu nigdy nie odzyskuje fraktalnych, zdrowych granic, stąd ocena radiologa oscylująca w zakresie 4--6. Zakres oceny automatycznej jest jeszcze mniejszy i wynosi po zaokrągleniu 4--5. Wynikiem tak niskich zakresów jest najgorszy wynik miary Corr spośród wszystkich sześciu parametrów, co oznacza, że drobne zmiany we fraktalności krawędzi nie są dobrze oceniane przez proponowaną metodę automatyczną. Również z uwagi na mały zakres zmian, MAE i MAX-AE osiągnęły drugi najlepszy wynik. Na tym etapie informatywność wynikająca z oceny tego parametru pozostaje wątpliwa i konieczna jest dalsza weryfikacja.
...
Jest to jedyny parametr oceniany w całości na zewnątrz ROI i charakteryzuje obrzęk w przestrzeni powięziowej. Na przełomie fazy poliferacji i remodelingu (6--9 tydz.) następuje charakterystyczny wzrost płynów w tym obszarze wynikający z intensywności procesu. W ocenach radiologa, szczególnie widoczny jest on dla pacjenta 115805 i 127752. Dla pacjenta 140283 widoczna jest stagnacja, a dla 127108 zwiększenie obrzęku, które utrzymuje się przez kolejne etapy do 26 tyg. W krzywych wygenerowanych w ocenie automatycznej również widoczne są maksima lokalne w okolicach przełomu faz gojenia się. Są one jednak przesunięte względem oceny radiologa (3--6 tydz.), co wymaga dalszej analizy.

Zbiorze podsumowanie powyższych wyników zostało przedstawione w formie omówienia macierzy błędów zaprezentowanych w kolejnej podsekcji.

\subsection{Macierze błędu}

W pierwszej kolejności zestawiono macierze błędu dla wszystkich 6 parametrów podsumowujące średni błąd absolutny dla oceny pacjentów testowych (zob. Rys. \ref{fig:CM_MAE}). 
\begin{figure}[t!]
	\centering
...
\end{figure}
Na wykresach zaznaczono ścieżkę przebiegającą wzdłuż minimalnych wartości. W optymalnym przypadku ścieżka powinna pokrywać się z diagonalą. Dla poszczególnych parametrów można zaobserwować dryf. 
2 najgorsze:
...
Na kolejnym wykresie (zob. Rys. \ref{fig:cmscores}) pokazano macierze błędu, gdzie klasą jest nota przyznawana w ocenie parametrów.
...
Największą zgodność otrzymano dla 2, 3 i 4. Dla 5 najczęściej oceniano jako 4 podobnie jak dla 6. Dla 7 oceniono albo 5 albo 6. Dla 1 najczęściej 2 lub 3, a 0 to 2.

Podsumowując prezentowane w Podsekcji \ref{seq:fusion} wyniki numeryczne i omówione w tej Sekcji krzywe gojenia oraz macierze błędu można wnioskować, że automatyczna metoda oceny procesu gojenia zachowuje trendy oceny radiologa, dla 4 parametrów osiągając Corr powyżej 0.7, a dla trzech powyżej lub równą 0.8. Niemniej jednak zastosowanie w algorytmie meta-regresji z wykorzystaniem maszyny wektorów nośnych spowodowało zmniejszeni zakresu oceny, która nigdy nie osiąga wartości skrajnych bliskich 7 lub 0--1. Minima i maksima lokalne również nie są tak charakterystyczna jak w przypadku oceny radiologa. Ponadto poprawa oceny parametru TU wymaga podejścia z uwzględnieniem przekrojów strzałkowych, a wartość parametru STE ponownej dyskusji w gronie ekspertów. W kolejnym Rozdziale zostaną zaprezentowane porównania opracowanej metody z innymi podejściami i przedyskutowane możliwości ich synergii.
