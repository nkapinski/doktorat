...
Obserwację gojącej się struktury ścięgna Achillesa umożliwiają techniki stosujące różnego rodzaju fale przenikające ciało ludzkie. Sposób w jaki propaguje się fala tworząc \textit{sygnał} zależny jest od własności związanych z ośrodkiem propagacji oraz parametrów fali. Do najistotniejszych należą wyrażana w hercach (w skr. Hz) \textit{częstotliwość}, czyli liczba pełnych cykli drgań w ciągu sekundy oraz \textit{amplituda} tj. maksymalna wartość sygnału falowego. Interpretacja zdarzeń wynikających ze zmian tych parametrów oraz reakcji materii na zadany sygnał falowy wykorzystywana jest w technikach obrazowania medycznego. 
