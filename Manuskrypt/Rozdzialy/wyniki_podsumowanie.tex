...
\centering
\caption{Wyniki szkolenia się sieci w paradygmacie end-to-end z wykorzystaniem zbioru treningowego. Pogrubieniem oznaczono najlepsze rezultaty, dodatkowo posługując się kolorem czerwonym w przypadku obserwacji średnich istotnych statystycznie z $p$ $<$ 0,05, w porównaniu z drugim najlepszym wynikiem.}
...
\begin{tabular}{lc||c|c|c|c|c|c}
	...
	Inception-v3$_{e}$ & MAE & 0,88$\pm$0,05 & \textbf{0,75}$\pm$0,04 & \textbf{0,82}$\pm$0,05 & \textbf{0,78}$\pm$0,03 & \textbf{0,91}$\pm$0,05 & \textcolor{red}{\textbf{0,67}}$\pm$0,04 \\
	...
	ResNet-50$_{e}$ & MAE & \textcolor{red}{\textbf{0,64}}$\pm$0,03 & 0,98$\pm$0,04 & 0,83$\pm$0,06 & 0,89$\pm$0,03 & 1,06$\pm$0,06 & 1,10$\pm$0,05  \\
...
Zdecydowanie najlepsze wyniki MAX-AE (5 z 6-ciu) uzyskano dla modelu Inception-v3$_{e}$. Wyjątkiem jest parametr SCT.

Otrzymane wartości MAE znajdują się w zakresie 0,64 do 1,10 (w skali wzorca odniesienia 0--7).  Najlepiej estymowanymi parametrami były TisE -- 0,67, TT -- 0,75 i TE -- 0,78. Najgorszy rezultat natomiast otrzymano dla TU -- 0,91, który to parametr silnie zależy od informacji widocznej w płaszczyźnie strzałkowej. Istotne statystycznie różnice uzyskano dla modelu Inception-v3$_{e}$ (TisE) i modelu ResNet-50$_{e}$ (SCT).

Najlepsze wyniki Corr dla wszystkich parametrów oprócz STE, dla sieci Inception-v3$_{e}$, przyjmują wartość powyżej 0,5. Niskie Corr dla STE wynika z niewielkich zmian trendu wartości tego parametru (często występuje stała wartość w dużym przedziale danych). W skrajnym przypadku, dla modelu ResNet-50$_{e}$, korelacja równa 0,00 wynika z predykcji stałych wartości dla całego okresu gojenia się i błędów numerycznych związanych z tym faktem. Dla tego modelu uzyskano również znacząco gorsze wyniki Corr w pozostałych przypadkach.

Podsumowując, kierując się powyższymi wynikami, do dalszych porównań wykorzystano model Inception-v3$_{e}$, który w Tab. \ref{tab:end-to-end_testset} zestawiono z proponowaną przez autora tej pracy metodą meta-regresji opartą o algorytm SVR (SVR), porównując wyniki na zbiorze testowych 4-ech pacjentów.  
\vspace{10px}
\renewcommand{\arraystretch}{1.2}
\begin{table*}[h]
...
Wyniki Corr świadczą, że cechy wykorzystywane w metodzie SVR do oceny procesu gojenia się skuteczniej dywersyfikują kolejne fazy procesu i pozwalają \linebreak na ocenę trendu z lepszą dokładnością niż metoda oparta o paradygmat end-to-end.   
