...
\label{CNNs}
\textit{Konwolucyjne sieci neuronowe} (ang. \textit{Convolutional Neural Networks}, CNN)\index{CNN -- skrót od ang. \textit{Convolutional Neural Networks}} \linebreak są biologicznie inspirowanymi sztucznymi sieciami neuronowymi. Należą do zbioru \textit{głębokich sieci neuronowych} (ang. \textit{Deep Neural Networks}, DNN)\index{DNN -- skrót od ang. \textit{Deep Neural Networks}}, które z kolei są podzbiorem \textit{systemów uczących się} (ang. \textit{Machine Learning Systems}) tj. algorytmów nie wymagających explicite programowania do ustalenia swoich parametrów. Algorytmy te często określane są obecnie wspomnianym we wstępie tej rozprawy terminem Wąskiej Sztucznej Inteligencji.
...
W odróżnieniu od naturalnych struktur biologicznych sieci konwolucyjne operują przeważnie na reprezentacji cyfrowej obrazu. Zapisywana jest ona w postaci tablicy liczb tzw. \textit{macierzy} o współrzędnych ($x$\index{$x$ -- pierwsza współrzędna prostokątna}, $y$\index{$y$ -- druga współrzędna prostokątna}), gdzie $x$ to kolumna macierzy, a \linebreak $y$ wiersz. W przypadku obrazów trójwymiarowych dochodzi jeszcze trzecia składowa $z$, a zamiast macierzy użyta jest wówczas ogólna postać odwzorowania wieloliniowego, czyli \textit{tensor}.

W obrazach cyfrowych dla każdego punktu ($x$, $y$), tzw. \textit{piksela}, kodowana jest informacja o wartości funkcji obrazowej $I$($x$, $y$)\index{$I$ -- funkcja obrazowa}. Na tej podstawie dzielimy obrazy na:
...
W sieciach konwolucyjnych, procesy zachodzące w korze wzrokowej są modelowane przy użyciu odpowiednio dobranych parametrów i funkcji służących do ekstrakcji istotnych informacji obrazowych i ich przetwarzania. Parametry wykorzystywane \linebreak do wstępnej ekstrakcji informacji grupowane są w \textit{filtry}, które realizują funkcję \textit{splotu} maski $K_m$\index{$K_m$ -- funkcja maski splotu} z kolejnymi fragmentami funkcji $I$. $K_m$ jest najczęściej macierzą kwadratową o wymiarze $N$, gdzie zazwyczaj $N$ = {1, 3, 5, 7, 9, 11}. Większe wymiary $K_m$ nie \linebreak są w praktyce stosowane z uwagi na rosnącą wraz z $N$ złożonością obliczeniową. Równanie splotu dyskretnego można zapisać następująco:
\begin{equation}
	I'\left(x, y\right) = \sum_{j=-n}^{n} \sum_{i=-k}^{k} I\left(x - j, y - i \right)K_m\left(j, i\right),
\end{equation}
gdzie $I'$\index{$I'$ -- funkcja obrazowa po operacji splotu z funkcją maski} jest nową funkcją obrazową powstałą po filtracji, a $i$\index{$i$, $j$, $k$ -- indeksy} i $j$ to kolejne współrzędne maski filtru w odniesieniu do jego punktu centralnego. Dla wartości brzegowych podczas realizacji splotu, dla współrzędnych poza obrazem, przyjmowane są z reguły zera. Rzadziej stosuje się inne metody takie jak: odbicie wartości $I$ obrazu poza jego granicami; powtórzenie wartości $I$ obrazu bez odbicia; modyfikację wymiaru maski filtru na brzegu obrazu, tak by nie wychodziła poza jego granicę.
...
\begin{itemize}[noitemsep,nolistsep]
	\item \textit{segmentację} -- podział obrazu na spójne fragmenty, najczęściej wiążące się \linebreak z wyodrębnieniem \textit{obiektu} $A_1$\index{$A_1$ -- obiekt zawarty w obrazie} z \textit{tła} $A_2$\index{$A_2$ -- tło zawarte w obrazie} na podstawie ustalonego progu lub miary np.:
	\begin{equation*}
	\begin{aligned}
	...
	\end{aligned}
	\end{equation*} 
	przy czym metody ustalania wartości progowej $T_p$\index{$T_p$ -- wartość progowe} lub wielu wartości {$T_p^1$,...,$T_p^n$} są obszarem szerokich badań (zob. \cite{DBLP:journals/corr/abs-1005-4020});
...
W nowym podejściu zarówno ekstrakcja cech jak i ostateczne wnioskowanie \linebreak na ich podstawie realizowane jest w jednym kroku, co nazywane jest paradygmatem \textit{end-to-end learning}. W kolejnej sekcji omówiono dokładniej  ewolucję tego podejścia. 

\section{Wprowadzenie}
\label{seq:DL_intro}
Pierwszy formalny model neuronu został zaproponowany przez Warrena McCulloch'a i Waltera Pitts'a w roku 1943 (zob. \cite{McCulloch1943}). Była to bramka logiczna, której wyjście stawało się aktywne w momencie, gdy liczba aktywnych wejść przekroczyła pewien zdefiniowany próg. Taka zależność sygnału wyjściowego $y_n$\index{$y_n$ -- sygnał wyjściowy neuronu} od sygnałów wejściowych $x_1$,...,$x_n$\index{$x_i$ -- sygnał wejściowy neuronu} została potem nazwana \textit{funkcją aktywacji neuronu}, którą zapisujemy jako:
\begin{equation}
\label{eqActFunc}
...
Zastosowanie dodatkowej jednowymiarowej tablicy współczynników, czyli \textit{wektora wag} $w$ = ($w_1$,...,$w_n$)\index{$w$ -- wektor wag} dało możliwość uczenia się poprzez adaptacyjną zmianę wartości poszczególnych jego elementów. W modelu Rosenblatta zastosowano ponadto progową funkcję aktywacji z progiem $T_a$\index{$T_a$ -- próg funkcji aktywacji}:
\begin{equation}
y_n(\Zeta)=\begin{cases} 0, & n < T_a, \\ 1, & n \ge T_a, \end{cases}
\end{equation}
gdzie $\Zeta$\index{$\Zeta$ -- suma ważona wyjść neuronów} to suma ważona wyjść poszczególnych neuronów:
\begin{equation}
\label{eqLinActFunc}
\Zeta=\sum_{i=1}^{n}w_i x_i
\end{equation}

Dla przykładu, w dwuwymiarowej przestrzeni sygnałów wejściowych {$x_1$, $x_2$}  działanie perceptronu można opisać jako wyliczenie funkcji liniowej rozdzielającej obserwacje $o_1$ od $o_2$\index{$o$ -- obserwacja}. W trzech wymiarach będzie to płaszczyzna, a w $n$-wymiarach, hiperpłaszczyzna. Z uwagi na swój liniowy charakter, perceptron proponowany przez Rosenblatta miał wiele ograniczeń, które szczegółowo sformułowali w 1969 roku Marvin Minsky i Seymour Papert w książce \textit{Perceptrons} (zob. \cite{Minsky1969}). Autorzy opublikowali listę problemów, których nie można było rozwiązać z użyciem perceptronu m.in. do najszerzej dyskutowanych należał przykład związany z brakiem możliwości modelowania \textit{funkcji XOR}, aktywującej wyjście przy aktywnym jednym z dwóch wejść.
...
\begin{itemize}[noitemsep,nolistsep]
	\item \textit{ekstraktor cech} -- część służąca do automatycznej ekstrakcji wektora cech \linebreak $w_c$\index{$w_c$ -- wektor cech}, zawierająca m.in. filtry.
	\item \textit{klasyfikator} -- część wykorzystywana do zadania wnioskowania końcowego \linebreak na podstawie $w_c$.
\end{itemize}  

Przy użyciu takiej architektury możliwe stało się zastosowanie paradygmatu end-to-end learning, w szczególności znalezienie w jednym procesie szkolenia transformacji, która obrazy przekształca bezpośrednio w ostateczną klasyfikację.  
...
\begin{figure}[h!]
	...
	\caption{Firmy stosujące w swoich produktach głębokie sieci neuronowe, uzyskujące certyfikację FDA (na podst. \cite{FDA}).\index{FDA -- skrót od ang. Food and Drug Administration}}
	\label{fda_cert}
\end{figure}
\newpage
...
Najbardziej zaawansowane prace dotyczą rozwiązania problemu gromadzenia dużych zbiorów danych medycznych, co wymaga bliskiej współpracy ekspertów medycznych z ekspertami od uczenia maszynowego. Często konieczna jest również modyfikacja bądź tworzenie dedykowanych programów do akwizycji danych medycznych. Jako przykłady takich inicjatyw można wymienić programy Stanford Medicine \cite{MedicalImageNet}, Harvard School of Medicine \cite{HMS} czy Massachusetts General Hospital, które w swoich repozytoriach zgromadziły już dziesiątki milionów zdjęć radiologicznych (za \cite{MGH}). Ponadto w roku 2018, na konferencji NVIDIA GTC\index{GTC -- skróT od ang. \textit{GPU Technology Conference}} (GPU Technology Conference) w San Jose (Kalifornia), Amerykańskie Stowarzyszenie Radiologii i stowarzyszenie MICCAI\index{MICCAI -- skrót od ang. \textit{Medical Image Computing and Computer Assisted Intervention}} (od ang. \textit{Medical Image Computing and Computer Assisted Intervention}) ogłosiły porozumienie, co do wspólnej współpracy mającej na celu eliminacje barier legislacyjnych związanych z pozyskiwaniem danych i wykorzystaniem algorytmów uczenia maszynowego.
...
Proces szkolenia się sieci nazywany jest również \textit{uczeniem się} lub \textit{treningiem}. Większość algorytmów szkolenia głębokich sieci neuronowych obejmuje zadanie \textit{optymalizacji}, rozumiane jako minimalizację, bądź maksymalizację \textit{funkcji celu} $f$($x$)\index{$f$($x$) -- funkcja celu} przez zmianę $x$. W literaturze można też znaleźć inne nazwy funkcji celu takie jak \textit{kryterium}, \textit{funkcja kosztów}, \textit{funkcja strat} lub \textit{funkcja błędów} (por. \cite{Goodfellow-et-al-2016}). 

Podczas zadania optymalizacji bardzo często wykorzystuje się \textit{pochodną funkcji} oznaczaną jako $f'$($x$)\index{$f'$($x$) -- pochodna funkcji celu} lub $\frac{\delta y}{\delta x}$\index{$\frac{\delta y}{\delta x}$ -- pochodna funkcji celu}, gdyż niesie ona informacje o nachyleniu funkcji \linebreak w punkcie $x$. W praktyce funkcje celu są wielowymiarowe dlatego wykorzystywane są \textit{pochodne cząstkowe} informujące o nachyleniu w poszczególnych wymiarach. Wektor zawierający pochodne cząstkowe funkcji nazywany jest \textit{gradientem} i oznaczany jest jako $\bigtriangledown f$($x$)\index{$\bigtriangledown f$($x$) -- gradient funkcji celu}. Wykorzystywane są również: macierz pochodnych cząstkowych tzw. \textit{macierz Jacobiego} oraz macierz \textit{drugich pochodnych} cząstkowych (tj. pochodnych pochodnych) tzw. \textit{macierz Hessego}.
...
\end{equation} 
gdzie $\epsilon$\index{$\epsilon$ -- szybkość uczenia się} to \textit{szybkość uczenia się} -- parametr określający wielkość kroku. Działanie to prowadzi do spadku wartości funkcji celu w kolejnych krokach. 
...
Aplikuje się je do tzw. \textit{reguły łańcuchowej}. Przypuśćmy, że $v_l$ = $g$($p_l$) i $u$ = $f$($g$($p_l$)) = $f$($v_l$), gdzie $p_l$\index{$p_l$ -- wektor wejściowy do reguły łańcuchowej} i $v_l$\index{$v_l$ -- wektor wejściowy do reguły łańcuchowej} to wektory. Wówczas regułę łańcuchową można zapisać jako:
\begin{equation}
...
Proces szkolenia się sieci ma na celu najlepsze możliwe przybliżenie docelowej klasyfikacji bazując na danych przykładach, czyli \textit{zbiorze uczącym} $U$\index{$U$ -- zbiór uczący}. Algorytmy optymalizacyjne używane do szkolenia głębokich sieci neuronowych zazwyczaj działają pośrednio, optymalizując pewną miarę wydajności $P$\index{$P$ -- miara wydajności szkolenia głębokich sieci neuronowych}, która jest zdefiniowana na \textit{zbiorze testowym}, zawierającym przykłady inne niż w $U$. Często bierze się również pod uwagę dodatkowy podzbiór, rozłączny z $U$ i $T$\index{$T$ -- zbiór testowy} -- \textit{zbiór walidacyjny}, który ma pomóc w wyborze najlepszych algorytmów i wartości parametrów wpływających na jakość szkolenia sieci.
...
Przykładem algorytmu minipakietowego jest \textit{stochastyczny spadek gradientu} (ang. \textit{Stochastic Gradient Descent}, SGD)\index{SGD -- skrót od ang. \textit{Stochastic Gradient Descent}}. Bazuje on na założeniu, że estymację gradientu można otrzymać wyliczając średnią gradientu z minipakietu $m$\index{$m$ -- minipakiet przykładów} przykładów. Kolejne kroki algorytmu można zapisać następująco:
\begin{enumerate}[noitemsep,nolistsep]
	...
	\item Oblicz estymację gradientu $g$ = $\frac{1}{m}\bigtriangledown \sum_{i=1}^{m}L_s(x_i, y_i, f)$, gdzie $L_s$\index{$L_s$ -- funkcja strat} to funkcja strat dla jednego przykładu o wejściowej wartości próbki $x_i$ i oczekiwanym wyjściu $y_i$.
...
\begin{enumerate}[noitemsep,nolistsep]
	\item Wybierz wartość początkową $\epsilon$, $\beta_1$, $\beta_2$ oraz ustaw początkowe wartości zmiennych momentu 1 i 2 stopnia $s_a$ = 0 i $r_a$ = 0, wartość kroku czasowego $t$ = 0\index{$t$ -- krok czasowy} \linebreak i stałą $\sigma$\index{$\sigma$ -- stała stabilizacji numerycznej} używaną do stabilizacji numerycznej.
	\item Wybierz minipakiet złożony z $m$ przykładów ze zbioru szkoleniowego.
	\item Oblicz estymację gradientu $g$\index{$g$ -- gradient} = $\frac{1}{m}\bigtriangledown \sum_{i=1}^{m}L_s(x_i, y_i, f)$ i zwiększ $t$ o 1.
	\item Aktualizuj estymację pierwszego momentu. $s_a$ $\mapsto$ $\beta_1$$s_a$ + (1-$\beta_1$)$g$.
	\item Aktualizuj estymację drugiego momentu. $r_a$ $\mapsto$ $\beta_2$$r_a$ + (1-$\beta_2$)$g\bigodot g$, gdzie $\bigodot$\index{$\bigodot$ -- iloczyn skalarny} to iloczyn skalarny dwóch gradientów.
...
Parametry:
\begin{itemize}[noitemsep,nolistsep]
	\item \textit{Fałszywie dodatnia klasyfikacja} (\textit{FP}\index{FP -- skrót od ang. \textit{False Positive}} od ang. \textit{False Positive}) -- liczba obserwacji zaklasyfikowanych jako pozytywne, a należących do klasy obserwacji negatywnych.
	\item \textit{Fałszywie ujemna klasyfikacja} (\textit{FN}\index{FN -- skrót od ang. \textit{False Negative}} od ang. \textit{False Negative}) -- liczba obserwacji zaklasyfikowanych jako negatywne, a należących do klasy obserwacji pozytywnych. 
	\item \textit{Prawdziwie pozytywna klasyfikacja} (\textit{TP}\index{TP -- skrót od ang. \textit{True Positive}} od ang. \textit{True Positive}) -- liczba wyników poprawnie zaklasyfikowanych jako pozytywne. 
	\item \textit{Prawdziwie negatywna klasyfikacja} (\textit{TN}\index{TN -- skrót od ang. \textit{True Negative}} od ang. \textit{True Negative}) -- liczba wyników poprawnie zaklasyfikowanych jako negatywne.
\end{itemize}

Miary:
\begin{itemize}[noitemsep,nolistsep]
	\item \textit{Dokładność klasyfikacji} (ang. \textit{Accuracy})\index{$ACC$ -- dokładność klasyfikacji} -- $ACC = \frac{TP + TN}{TP+TN+FP+FN}$.
	\item \textit{Czułość klasyfikacji} (ang. \textit{Sensitivity})\index{$TPR$ -- czułość klasyfikacji} -- $TPR = \frac{TP}{TP + FN}$.
	\item \textit{Swoistość klasyfikacji} (ang. \textit{Specificity})\index{$TNR$ -- swoistość klasyfikacji} -- $TNR = \frac{TN}{TN + FP}$.
	\item \textit{Wartość predykcyjna dodatnia} (ang. \textit{Precision})\index{$PPV$ -- Wartość predykcji dodatniej klasyfikacji} -- $PPV = \frac{TP}{TP + FP}$.
	\item \textit{Wartość predykcyjna ujemna} (ang. \textit{Negative precision})\index{$NPV$ -- Wartość predykcji ujemnej klasyfikacji} -- $NPV = \frac{TN}{TN + FN}$.
\end{itemize}
...
\begin{itemize}[noitemsep,nolistsep]
\item \textit{Średni błąd obciążenia} (ang. Mean Bias Error)\index{$MBE$ -- średni błąd obciążenia} -- $MBE=\frac1N\sum_{i=1}^N{(x_{f,i}-x_{o,i})}$;
\item \textit{Średni błąd absolutny}\index{$MAE$ -- średni błąd absolutny} (ang. Mean Absolute Error) --  $MAE=\frac1N\sum_{i=1}^N\left|x_{f,i}-x_{o,i}\right|$;
\item \textit{Średni błąd kwadratowy} (ang. Root Mean Square Error)\index{$RMSE$ -- średni błąd kwadratowy} -- $RMSE=\sqrt{\frac1N\sum_{i=1}^N\left(x_{f,i}-x_{o,i}\right)^2}$;
\end{itemize}
oraz błędy maksymalne:
\begin{itemize}[noitemsep,nolistsep]
	\item \textit{Maksymalny błąd obciążenia} (ang. Maximum Bias Error)\index{$MAX_{BE}$ -- maksymalny błąd obciążenia} -- $MAX_{BE}=max_{i=1}^N{(x_{f,i}-x_{o,i})}$;
	\item \textit{Maksymalny błąd absolutny} (ang. Maximum Absolute Error)\index{$MAX_{AE}$ -- maksymalny błąd absolutny} --  $MAX_{AE}=max_{i=1}^N\left|x_{f,i}-x_{o,i}\right|$;
	\item \textit{Maksymalny błąd kwadratowy} (ang. Maximum Root Square Error)\index{$MAX_{RMSE}$ -- maksymalny błąd kwadratowy} -- $MAX_{RMSE}=max_{i=1}^N\sqrt{\left(x_{f,i}-x_{o,i}\right)^2}$;
\end{itemize}
...
$Corr=\frac1N\sum_{i=1}^N{ZF(r_{f,i})}$\index{$Corr$ -- uśredniona korelacja z wykorzystaniem transformacji Z-Fishera}  
uzyskując średnią zgodność estymacji i wzorca odniesienia. Przy czym $r_f$\index{$r_f$ -- współczynnik korelacji Pearsona} to zapisany wzorem \ref{PearsorR} współczynnik korelacji Pearsona. 

Oprócz prawidłowego wyboru metryki istotny jest również dobór możliwie efektywnej architektury sieci. Problemy z tym związane zostaną omówione w kolejnych podsekcjach.
...
\end{equation}
gdzie $A$\index{$A$ -- macierz przekształcenia liniowego} jest macierzą przekształcenia liniowego, a $b$\index{$b$ -- wektor przesunięcia} wektorem przesunięcia. Jako przykłady takich przekształceń dla dwuwymiarowych obrazów można wymienić:
\begin{itemize}[noitemsep,nolistsep]
	\item \textit{rotację} -- obrót obrazu o kąt $\theta$\index{$\theta$ -- kąt obrotu obrazu}, gdzie:
	\begin{equation}
	A = \begin{bmatrix} 
	\cos \theta & \sin \theta\\
	-\sin \theta & \cos \theta
	...
	\end{bmatrix}
	\end{equation}
	\item \textit{skalowanie} -- zmiana rozmiaru obrazu o $S$\index{$S$ -- skala zmiany obrazu}, gdzie:
	\begin{equation}
			A = \begin{bmatrix} 
		S_x & 0\\
		0 & S_y
...
\item przekształcenie oryginalnych $n$\index{$n$ -- liczba zmiennych} zmiennych na nowy zbiór $n'$ cech, gdzie ponownie $n'$ $<<$ $n$.
\end{itemize}
...
\begin{itemize}[noitemsep,nolistsep]
	\item \textit{miary siły związku} -- określające podobieństwo między rozkładami zmiennych losowych. Najczęściej stosowana jest \textit{korelacja Pearsona}, której współczynnik $r$\index{$r$ -- współczynnik korelacji} dla dwóch zmiennych losowych $X1$\index{$X1$, $X2$ -- zmienne losowe} i $X2$, będących dwoma równolicznymi zbiorami sparowanych, obserwowanych wartości, zapisywany jest następującym wzorem:
	\begin{equation}
	\label{PearsorR}
	...
	\end{equation}
	gdzie $X1_i$, $X2_i$ to wartości kolejnych obserwacji, a $\overline{X1}$ i $\overline{X2}$\index{$\overline{X}$ -- średnia z wartości kolejnych obserwacji} to ich średnie. Innymi słowy tak zapisany współczynnik $r_{X1X2}$ jest ilorazem \textit{kowariancji} \linebreak i iloczynu \textit{odchyleń standardowych} zmiennych $X1$ i $X2$.
	\item \textit{miary entropii względnej} --  określające rozbieżności między rozkładami zmiennych losowych. Najczęściej stosowana jest \textit{dywergencja Kullbacka-Leiblera}, której współczynnik $d_{KL}$\index{$d_{KL}$ -- współczynnik dywergencji Kullbacka-Leiblera} dla dwóch porównywanych dyskretnych rozkładów prawdopodobieństwa $p_k$ i $q_k$\index{$p_k$, $q_k$ -- rozkłady prawdopodobieństwa} zapisany jest wzorem: 
	\begin{equation}
...
W celu selekcji cech można również stosować różnego rodzaju, specjalnie zaprojektowane regresje. Do najbardziej powszechnie stosowanych należą algorytmy regresji grzbietowej, LASSO\index{LASSO -- skrót od ang. \textit{Least Absolute Shrinkage and Selection Operator}} (od ang. \textit{Least Absolute Shrinkage and Selection Operator}) oraz Elastic Net. Różnią się one definicją zadania minimalizacji funkcji kosztu poprzez optymalizację wektora współczynników regresji pełniącego jednocześnie funkcję wektora cech $w$:  
...
\begin{enumerate}[noitemsep,nolistsep]
\item Oblicz macierz kowariancji: $S_x$ = $X^{T}X$, gdzie $X$ to macierz danych zawierająca obserwacje w wierszach. Macierz $S_x$\index{$S_x$ -- macierz kowariancji} jest symetryczna i pozwala ocenić: (1) wariancje zmiennych poprzez analizę elementów na głównej przekątnej; (2) zależności pomiędzy zmiennymi poprzez analizę elementów poza główną przekątną. 
\item Dokonaj diagonalizacji: $S_x$ = $KL_dK^{-1}$, gdzie $L_d$\index{$L_d$ -- macierz diagonalna} to macierz diagonalna, a $K$\index{$K$ -- macierz odwracalna} to macierz odwracalna składająca się z wektorów własnych odpowiadających kolejnym wartościom własnym.
\item Utwórz macierz nowych zmiennych wykonując operację: $Y$ = $XK$\index{$Y$ -- macierz nowych zmiennych PCA}
\end{enumerate}

Oprócz PCA, w kontekście redukcji wymiarowości, można również wyróżnić następujące algorytmy: 
\begin{enumerate}[noitemsep,nolistsep]
	\item \textit{Multidimensional scaling}, MDS\index{MDS -- skrót od ang. \textit{Multidimensional scaling}} -- bazuje na wyliczeniu odpowiednio zdefiniowanego dystansu między wartościami cech w oryginalnej przestrzeni i próbie zachowania tego dystansu w zredukowanej wymiarowości (zob. \cite{Chen:2008:HDV:1370965}).
	\item \textit{T-distributed stochastic neighbor embedding}, t-SNE\index{t-SNE -- skrót od ang. \textit{T-distributed stochastic neighbor embedding}} -- stosowany do wizualizacji struktur cech w różnych skalach (zob. \cite{vanDerMaaten2008}).
	...
	\item \textit{Independent Component Analysis}, ICA\index{ICA -- skrót od ang. \textit{Independent Component Analysis}} -- stosowany do znalezienia reprezentacji danych składających się z niezależnych elementów (zob. \cite{ICA}).
	\item \textit{Latent Semantic Analysis}, LSA\index{LSA -- skrót od ang. \textit{Latent Semantic Analysis}} -- stosowany głównie w problemach dotyczących przetwarzania języka naturalnego, zwłaszcza do znalezienia kontekstu użycia danego słowa poprzez analizę statystyczną dużych bloków tekstów (zob. \cite{Wolfe2003}). 
	\item \textit{Self Organizing Maps}, SOM\index{SOM -- skrót od ang. \textit{Self Organizing Maps}} --  bazuje na zachowaniu własności topologicznych przestrzeni cech oryginalnych w zredukowanej wymiarowości (zob. \cite{Vesanto00self-organizingmap}).
\end{enumerate}
...
\begin{itemize}[noitemsep,nolistsep]
	...
	\item \textit{warstwy gęste lub z ang. fully connected}, FC\index{FC -- skrót od ang. \textit{Fully Connected}} -- składające się z neuronów, \linebreak z których każdy jest połączony z każdym neuronem kolejnej warstwy. Występują zazwyczaj w ostatniej części sieci konwolucyjnych tj. w klasyfikatorze;
...
Postęp w rozwoju współczesnych sieci konwolucyjnych doskonale odzwierciedla progres w rezultatach konkursu ImageNet Large Scale Visual Recognition Competition (w skr. ILSVRC)\index{ILSVRC -- skrót od ang. \textit{ImageNet Large Scale Visual Recognition Competition}} przedstawiony na Rys. \ref{ILSVRC}.
\begin{figure}[h!]
	\centering
...
\item Rezultat w postaci macierzy $C_a$ ponownie przewymiarowany jest na postać [55$\times$55$\times$96].\index{$A_a$, $B_a$, $C_a$ -- macierze przetwarzane przez sieć AlexNet}  
\end{enumerate} 
...
\end{split}
\end{equation}
gdzie $x_l$ i $x_{l+1}$ stanowią sygnał wejściowy i wyjściowy $l$-tego bloku. $F$\index{$F$ -- funkcja rezydualna} stanowi funkcję rezydualną optymalizowaną podczas treningu sieci, $h$($x_l$)\index{$h$($x_l$) -- funkcja przekształcenia sygnału} stanowi funkcję przekształcenia sygnału $x_l$ przekazywanego skrótowym połączeniem, $f$ jest funkcją ReLU, \linebreak a $W$\index{$W$ -- macierz wag} stanowi macierz wag.

Funkcja $h$($x_l$) jest funkcją tożsamościową, a zatem $h$($x_l$) = $x_l$. Żeby uzasadnić ten wybór należy rozważyć propagację gradientu wewnątrz sieci składającej się \linebreak z bloków rezydualnych. Dla każdego $L$-tego\index{$L$ - liczba bloków rezydualnych} bloku zachodzi równanie:
\begin{equation}
...
\end{equation}
z czego wynika, że gradient może być podzielony na dwie addytywne składowe\index{$w_r$ -- gradient propagowany w ResNet}: (1) $w_r$ = $\frac{\partial \varepsilon}{\partial x_L}$ propagowaną bez wpływu na warstwy zawierające wagi i (2) $\lambda_r$ = $\frac{\partial \varepsilon}{\partial x_L}\frac{\partial }{\partial x_l}\sum_{i=l}^{L-1}F(x_i, W_i)$ propagowaną przez nie.
...
Wartości $w_r$ są zazwyczaj znormalizowane do przedziału (-1;1) można więc rozważyć 4 istotne przypadki równania \ref{gradRes3Block}:
\begin{enumerate}[noitemsep,nolistsep]
	...
	\item $\lambda_r$ $<$ 1 -- przy założeniu, że $w_r$ + $\lambda_r$ $<$ 1, dla sieci składających się z wielu warstw występuje problem zaniku gradientu, jak w przypadku 1. Natomiast, gdy \linebreak $w_r$ + $\lambda_r$ $>$ 1, podobnie jak w przypadku 2, może występować problem eksplozji gradientu.
...
\end{equation} 
\index{$\sigma (z_s)_j$ -- funkcja softmax}używanej często bezpośrednio przed ostatnią warstwą sieci neuronowych dla $z_s$\index{$z_s$ -- liczba sygnałów wejściowych funkcji softmax} sygnałów wejściowych i $j$\index{$j$ -- liczba sygnałów wyjściowych funkcji softmax} wyjściowych. Co istotne, rezultaty funkcji softmax przy przyjęcia odpowiedniego modelu probabilistycznego mogą stanowić prawdopodobieństwa przynależności do odpowiednich klas.
...
Idea generalizacji stosów oryginalnie została zaproponowana w \cite{Wolpert92stackedgeneralization}. Wykorzystana została koncepcja \textit{meta-uczenia}, a zatem konstrukcja nadrzędnej metody, której zadaniem jest wybór optymalnego wektora wag $w$\index{$w$ -- wektor wag} dla stosu $S^g$\index{$S^g$ -- stos predykcji} predykcji dla danych $x_s$:
\begin{equation}
