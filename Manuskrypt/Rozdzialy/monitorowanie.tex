...
Pierwszą opisaną w tej pracy metodą obrazowania stosowaną do monitorowania gojenia się ścięgna Achillesa jest obrazowanie z wykorzystaniem \textit{Jądrowego Rezonansu Magnetycznego} nazywanego dla uproszczenia \textit{Rezonansem Magnetycznym}, w skr. \textit{RM} (ang. \textit{Magnetic Resonance Imaging}, w skr. \textit{MRI}). Metoda ta bazuje na odpowiednim odczycie reakcji jąder atomów na stałe pole magnetyczne generowane przez magnesy, zmienne pole magnetyczne skierowane prostopadle do osi stałego pola (generowane przez fale elektromagnetyczne wytwarzane w cewce), zmienne pole lokalne generowane przez sąsiednie jądra atomowe oraz związane z nimi cząstki elementarne.

Zrozumienie tych procesów było możliwe dzięki badaniom naukowym z początku XX wieku, które zapewniły wystarczające do wykorzystania w praktyce zrozumienie procesów definiujących zachowania obiektów o bardzo małych masach i wymiarach takich jak cząstki elementarne. Jako przykłady można wskazać prace Alberta Einsteina dotyczące ruchów Browna oraz efektu fotoelektrycznego; opis funkcji falowej zaproponowany przez Erwina Shrodingera; badania Nielsa Bohra, które doprowadziły do utworzenia utylitarnego modelu atomu zawierającego poziomy energetyczne; wyprowadzenie reguły nieoznaczoności przez Werner Heisenberga, czy opisanie przez Wolfganga Pauliego konfiguracji cząstek elementarnych w atomie. 

Teoria mechaniki kwantowej, która rozwinęła się na podstawie m.in. powyższych prac umożliwiła zrozumienie magnetycznej natury \textit{protonów}, czyli cząstek elementarnych, które wraz z \textit{neutronami} tworzą jądro atomowe. W 1933 r. Frisch i Stern w \cite{Frisch1933} opisali moment magnetycznego protonu. Rok pózniej, w \cite{Breit1934} Breit i Rabi zaproponowali metodę rezonansową obserwacji własności magnetycznych jąder atomowych. W 1946 metoda ta została udoskonalona i wykorzystana przez Felix Bloch'a do badania cząstek w wodzie i parafinie \cite{Bloch1946}. Z kolei dzięki pracom m.in. Lautenrbur'a \cite{LAUTERBUR1973} i Mansfield'a \cite{Mansfield1977} rozwinęły się metody lokalizacji przestrzennej powyższych zmian własności magnetycznych, co umożliwiło prezentację wizualną tak zbieranych informacji. 
