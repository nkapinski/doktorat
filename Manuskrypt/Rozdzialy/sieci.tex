...
Wprowadzonie w ostatnich latach dedykowanych dla głębokiego uczenia się rozwiązań zarówno w warstwie sprzętowej jak i oprogramowania umożliwiło tworzenie architektur o wysokim stopniu komplikacji. W głównej mierze rozwiązania te umożliwiły optymalizacji fazy szkolenia omówionej w poprzedniej sekcji jak również \textit{fazy wnioskowania}, gdzie wytrenowana sieć użyta jest do przetwarzania kolejnych obserwacji. Przede wszystkim można wyróżnić dedykowane akceleratory do operacji macierzowych takie jak \textit{NVIDIA Tensor Processing Unit} (w skr. \textit{TPU}) [] czy \textit{Intel Lakecrest} [] oraz rozwiązania takie jak [24-TTCF] minimalizują czasy przetwarzania sygnału przeez wytrenowaną sieć np. stosując optymalizację reprezentacji liczb zmiennoprzecinkowych lub przetwarzanych macierzy.

Dodatkowo zostało również opracowanych wiele framework'ów do obliczeń z udziałem głębokich sieci neuronowych takich jak: Caffe, Caffe2, TensorFlow, Theano, PyTorch czy MXNet (zob. [porównania]). W kolejnych podsekcjach opisano szczegółowo wybrane architektury, które znajdują się w implementacjach powyższych framework'ów i charakteryzują się wprowadzeniem pewnych innowacyjnych rozwiązań. 
