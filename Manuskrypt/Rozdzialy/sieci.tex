...
\end{equation}
gdzie $I'$\index{$I'$ -- funkcja obrazowa po operacji splotu z funkcją maski} jest nową funkcją obrazową powstałą po filtracji, a $i$\index{$i$ -- pierwsza współrzędna maski filtru} i $j$\index{$j$ -- druga współrzędna maski filtru} to kolejne współrzędne maski filtru w odniesieniu do jego punktu centralnego. Dla wartości brzegowych podczas realizacji splotu, dla współrzędnych poza obrazem, przyjmowane są z reguły zera. Rzadziej stosuje się inne metody takie jak: odbicie wartości $I$ obrazu poza jego granicami; powtórzenie wartości $I$ obrazu bez odbicia; modyfikację wymiaru maski filtru na brzegu obrazu, tak by nie wychodziła poza jego granicę.
...
\label{seq:DL_intro}
Pierwszy formalny model neuronu został zaproponowany przez Warrena McCulloch'a i Waltera Pitts'a w roku 1943 (zob. \cite{McCulloch1943}). Była to bramka logiczna, której wyjście stawało się aktywne w momencie, gdy liczba aktywnych wejść przekroczyła pewien zdefiniowany próg. Taka zależność sygnału wyjściowego $y$\index{$y$ -- sygnał wyjściowy} od sygnałów wejściowych $x_1$,...,$x_n$\index{$x$ -- sygnał wejściowy} została potem nazwana \textit{funkcją aktywacji neuronu}, którą zapisujemy jako:
\begin{equation}
\label{eqActFunc}
y=f\left(x_1, x_2,..., x_n\right)\index{$y$ -- funkcja aktywacji neuronu}
\end{equation}
...
\begin{itemize}[noitemsep,nolistsep]
	\item \textit{ekstraktor cech} -- część służąca do automatycznej ekstrakcji wektora cech \linebreak $w$\index{$w$ -- wektor cech}, zawierająca m.in. filtry.
...
Aplikuje się je do tzw. \textit{reguły łańcuchowej}. Przypuśćmy, że $v$ = $g$($p$) i $u$ = $f$($g$($p$)) = $f$($v$), gdzie $p$\index{$p$ -- wektor wejściowy do reguły łańcuchowej} i $v$\index{$v$ -- wektor wejściowy do reguły łańcuchowej} to wektory. Wówczas regułę łańcuchową można zapisać jako:
\begin{equation}
...
\begin{enumerate}[noitemsep,nolistsep]
	...
	\item Oblicz estymację gradientu $g$ = $\frac{1}{m}\bigtriangledown \sum_{i=1}^{m}L(x_i, y_i, f)$, gdzie $L$\index{$L$ -- funkcja strat} to funkcja strat dla jednego przykładu o wejściowej wartości próbki $x_i$ i oczekiwanym wyjściu $y_i$.
...
\begin{enumerate}[noitemsep,nolistsep]
	...
	\item Wybierz minipakiet złożony z $m$ przykładów ze zbioru szkoleniowego.
	\item Oblicz estymację gradientu $g$\index{$g$ -- gradient} = $\frac{1}{m}\bigtriangledown \sum_{i=1}^{m}L(x_i, y_i, f)$ i zwiększ $t$ o 1.
	\item Aktualizuj estymację pierwszego momentu. $s_a$ $\mapsto$ $\beta_1$$s_a$ + (1-$\beta_1$)$g$.
	\item Aktualizuj estymację drugiego momentu. $r_a$ $\mapsto$ $\beta_2$$r_a$ + (1-$\beta_2$)$g\bigodot g$, gdzie $\bigodot$\index{$\bigodot$ -- iloczyn skalarny dwóch grdientów} to iloczyn skalarny dwóch gradientów.
...
\begin{itemize}[noitemsep,nolistsep]
	...
	\end{equation}
	gdzie $X1_i$, $X2_i$ to wartości kolejnych obserwacji\index{$X_i$ -- wartość kolejnych obserwacji}, a $\overline{X1}$ i $\overline{X2}$\index{$\overline{X}$ -- średnia z wartości kolejnych obserwacji} to ich średnie. Innymi słowy tak zapisany współczynnik $r_{X1X2}$ jest ilorazem \textit{kowariancji} \linebreak i iloczynu \textit{odchyleń standardowych} zmiennych $X1$ i $X2$.
...
\item Dokonaj diagonalizacji: $S_x$ = $KLK^{-1}$, gdzie $L$\index{$L$ -- macierz diagonalna} to macierz diagonalna, a $K$\index{$K$ -- macierz odwracalna} to macierz odwracalna składająca się z wektorów własnych odpowiadających kolejnym wartościom własnym.
...
Funkcja $h$($x_l$) jest funkcją tożsamościową, a zatem $h$($x_l$) = $x_l$. Żeby uzasadnić ten wybór należy rozważyć propagację gradientu wewnątrz sieci składającej się \linebreak z bloków rezydualnych. Dla każdego $L$-tego\index{$L$ - blok rezydualny} bloku zachodzi równanie:
\begin{equation}
...
\end{equation}
z czego wynika, że gradient może być podzielony na dwie addytywne składowe\index{$w$ -- gradient propagowany w ResNet}: (1) $w$ = $\frac{\partial \varepsilon}{\partial x_L}$ propagowaną bez wpływu na warstwy zawierające wagi i (2) $\lambda$ = $\frac{\partial \varepsilon}{\partial x_L}\frac{\partial }{\partial x_l}\sum_{i=l}^{L-1}F(x_i, W_i)$ propagowaną przez nie.
...
\end{equation} 
\index{$\sigma (z)_j$ -- funkcja softmax}używanej często bezpośrednio przed ostatnią warstwą sieci neuronowych dla $z$\index{$z$ -- liczba sygnałów wejściowych funkcji softmax} sygnałów wejściowych i $j$\index{$j$ -- liczba sygnałów wyjściowych funkcji softmax} wyjściowych. Co istotne, rezultaty funkcji softmax przy przyjęcia odpowiedniego modelu probabilistycznego mogą stanowić prawdopodobieństwa przynależności do odpowiednich klas.
