...
W nowym podejściu zarówno ekstrakcja cech jak i ostateczne wnioskowanie \linebreak na ich podstawie realizowane jest w jednym kroku, co nazywane jest paradygmatem \textit{end-to-end learning}. 
...
Aplikuje się je do tzw. \textit{reguły łańcuchowej}. Dla przykładu, jeżeli $v_l$ = $g$($p_l$) i $u$ = $f$($g$($p_l$)) = $f$($v_l$), gdzie $p_l$\index{$p_l$ -- wektor wejściowy do reguły łańcuchowej} i $v_l$\index{$v_l$ -- wektor wejściowy do reguły łańcuchowej} to wektory. Wówczas regułę łańcuchową można zapisać jako:
\begin{equation}
...
Proces szkolenia się sieci ma na celu najlepsze możliwe przybliżenie docelowego zadania bazując na danych przykładach, czyli \textit{zbiorze uczącym} $U$\index{$U$ -- zbiór uczący}. Algorytmy optymalizacyjne używane do szkolenia głębokich sieci neuronowych zazwyczaj działają pośrednio, optymalizując pewną miarę wydajności $P$\index{$P$ -- miara wydajności szkolenia głębokich sieci neuronowych}, która jest zdefiniowana na \textit{zbiorze testowym} $T$, zawierającym przykłady inne niż w $U$. Często bierze się również pod uwagę dodatkowy podzbiór, rozłączny z $U$ i $T$\index{$T$ -- zbiór testowy} -- \textit{zbiór walidacyjny}, który ma pomóc w wyborze najlepszych algorytmów i wartości parametrów wpływających na jakość szkolenia sieci.
...
\begin{enumerate}[noitemsep,nolistsep]
	...
	\item Oblicz estymację gradientu $g$ = $\frac{1}{m}\bigtriangledown \sum_{i=1}^{m}L_s(x_i, y_i, f)$, gdzie $L_s$\index{$L_s$ -- funkcja strat} to funkcja strat dla jednego przykładu o wejściowej wartości $x_i$ i oczekiwanym wyjściu $y_i$.
...
\begin{enumerate}[noitemsep,nolistsep]
	\item Wybierz wartość początkową $\epsilon$, $\beta_1$, $\beta_2$ oraz ustaw początkowe wartości zmiennych momentu 1-ego i 2-ego stopnia $s_a$ = 0 i $r_a$ = 0, wartość kroku czasowego $t$ = 0\index{$t$ -- krok czasowy} i stałą $\sigma$\index{$\sigma$ -- stała stabilizacji numerycznej} używaną do stabilizacji numerycznej.
	\item Wybierz minipakiet złożony z $m$ przykładów ze zbioru szkoleniowego.
...
\end{equation}\index{$Corr$ -- uśredniona korelacja z wykorzystaniem transformacji Z-Fishera} uzyskując średnią zgodność estymacji i wzorca odniesienia. Przy czym $r_f$\index{$r_f$ -- współczynnik korelacji Pearsona} to zapisany wzorem \ref{PearsorR} współczynnik korelacji Pearsona. 
...
\begin{itemize}[noitemsep,nolistsep]
	\item \textit{miary siły związku} -- określające podobieństwo między rozkładami zmiennych losowych. Najczęściej stosowana jest \textit{korelacja Pearsona}, której współczynnik $r$\index{$r$ -- współczynnik korelacji} dla zmiennych losowych $X1$\index{$X1$, $X2$ -- zmienne losowe} i $X2$, będących równolicznymi zbiorami sparowanych, obserwowanych wartości, zapisywany jest następującym wzorem:
	\begin{equation}
	\label{PearsorR}
...
Współczesne sieci konwolucyjne służące do analizy obrazów, podobnie jak u protoplasty tj. sieci LeNet, składają się z dwóch głównych części: ekstraktora cech \linebreak i klasyfikatora. W porównaniu jednak do pierwszych topologii, różnorodność warstw uległa zwiększeniu. Do najczęściej stosowanych komponentów można zaliczyć:
\begin{itemize}[noitemsep,nolistsep]
...
\end{equation} 
\index{$\sigma (z_s)_j$ -- funkcja softmax}używanej często bezpośrednio przed ostatnią warstwą sieci neuronowych dla $z_s$\index{$z_s$ -- liczba sygnałów wejściowych funkcji softmax} sygnałów wejściowych i $j$\index{$j$ -- liczba sygnałów wyjściowych funkcji softmax} wyjściowych. Co istotne, rezultaty funkcji softmax przy przyjęciu odpowiedniego modelu probabilistycznego mogą stanowić prawdopodobieństwa przynależności do odpowiednich klas.
