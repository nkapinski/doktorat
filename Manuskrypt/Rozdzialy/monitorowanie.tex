...
Czas potrzebny na zwiększenie magnetyzacji podłużnej do stanu z przed działania impulsu nazywany jest czasem \textit{relaksacji podłużnej} lub \textit{relaksacji spin -- sieć}. Z kolei czas potrzebny na obniżenie magnetyzacji poprzecznej nazywany jest czasem \textit{relaksacji poprzecznej} lub \textit{relaksacji spin -- spin}. Relaksacja spin--spin ma przede wszystkim znaczenie w kontekście oddziaływań z sąsiednimi jądrami i występującą na skutek tego dyfuzją fazy obniżającą wartość magnetyzacji poprzecznej. Istotnymi parametrami obu powyższych procesów są odpowiednio stałe $T1$ i $T2$, określające dynamikę omówionych zmian. Z reguły $T2$ $\leq$ $2T1$.  
